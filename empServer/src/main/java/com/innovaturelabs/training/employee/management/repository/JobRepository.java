...
public interface JobRepository extends Repository<Job, Integer> {
    ...
    @Query(value = "SELECT * FROM job_tbl WHERE status IN ?1 AND qualification<=?2 AND(title LIKE %?3% OR description LIKE %?3% )", nativeQuery = true)
    Page<Job> findAllByStatusAndQualification(ArrayList<Byte> status, Byte qualification,String search, Pageable page);
